...
// 1. Add route rendering logic to the App object or globally
...
// Insert the new methods after updateMapLocation (around line 4554)
// We can just append it to the App object somewhere, or right after calculateDeadline
appJs = appJs.replace(/calculateDeadline\(\) \{/, renderRoutesLogic + '\n  calculateDeadline() {');

// 2. Replace the calculatePrevisionGoogle call logic
...
const calcPrevisionIfOld = `if (routeData) {
...
const calcPrevisionIfNew = `if (routesList && routesList.length > 0) {
...
appJs = appJs.replace(calcPrevisionIfOld, calcPrevisionIfNew);
...
// 3. Replace runGoogle_AIRouting logic
const runGoogleOld = `const routeData = await window.LocationService.calculateRoute(originCoords, destCoords);
        if (routeData) {
          document.getElementById('plan-dist').value = Math.round(routeData.distanceKm);
...
const runGoogleNew = `const routesList = await window.LocationService.calculateRoute(originCoords, destCoords);
...
appJs = appJs.replace(runGoogleOld, runGoogleNew);
...
// Add event listener for routeSelected in app initialization
const eventListener = `
...
`;
appJs = appJs.replace(/setupEventListeners\(\) \{/, 'setupEventListeners() {' + eventListener);
